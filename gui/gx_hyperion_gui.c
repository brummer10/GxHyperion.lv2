...
void controller_expose(gx_hyperionUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
void resize_event(gx_hyperionUI *ui) {
...
void scroll_event(gx_hyperionUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
void button1_event(gx_hyperionUI *ui, double* start_value) {
	for (int i=0;i<CONTROLS;i++) {
		if (aligned(ui->pos_x, ui->pos_y, &ui->controls[i], ui)) {
...
void set_key_value(gx_hyperionUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
void key_event(gx_hyperionUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
void set_previous_controller_active(gx_hyperionUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
void set_next_controller_active(gx_hyperionUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
void motion_event(gx_hyperionUI *ui, double start_value, int m_y) {
