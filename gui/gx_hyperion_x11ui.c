...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_hyperionUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_hyperionUI* ui = (gx_hyperionUI*)malloc(sizeof(gx_hyperionUI));
	...
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 40, 30, 71, 71}, false,"INTENSITY", KNOB, INTENSITY};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 150, 30, 71, 71}, false,"VOLUME", KNOB, VOLUME};
...
static void bypass_expose(gx_hyperionUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_hyperionUI *ui, gx_controller* controller) {
...
static void _expose(gx_hyperionUI *ui) {
	static const char* plug_name = "GxHyperion" ;
...
static void controller_expose(gx_hyperionUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void check_value_changed(gx_hyperionUI *ui, int i, float* value) {
...
static void check_is_active(gx_hyperionUI *ui, int i, bool set) {
...
bool aligned(int x, int y, gx_controller *control, gx_hyperionUI *ui) {
...
static void scroll_event(gx_hyperionUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
static void set_key_value(gx_hyperionUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
static void key_event(gx_hyperionUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
static void set_previous_controller_active(gx_hyperionUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
static void set_next_controller_active(gx_hyperionUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
static void motion_event(gx_hyperionUI *ui, double start_value, XMotionEvent *mo) {
