...
#define GTK_HYP_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_HYP_KNOB, GtkHypKnobPrivate))

G_DEFINE_TYPE(GtkHypKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkHypKnobPrivate GtkHypKnobPrivate;

struct _GtkHypKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkHypKnobPrivate *priv = GTK_HYP_KNOB_GET_PRIVATE(widget);
...
{
	g_assert(GTK_IS_HYP_KNOB(widget));
	GtkHypKnob *knob = GTK_HYP_KNOB(widget);
	GtkHypKnobPrivate *priv = GTK_HYP_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkHypKnobClass
 */

static void gtk_knob_class_init (GtkHypKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkHypKnobPrivate));
}
...
static void gtk_knob_init (GtkHypKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_HYP_KNOB, NULL ));
